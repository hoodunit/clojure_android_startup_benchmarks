...
(defn get-start-end-times [filename start-activity main-activity]
  (try (with-open [rdr (io/reader filename)]
...
(defn print-file-results [filename {:keys [start-activity main-activity]}]
  (if-let [start-end (get-start-end-times filename start-activity main-activity)]
    (let [times (get-run-times start-end)]
      (println filename)
      (doseq [t times] (println t))
...
(defn print-test-results [{:keys [test-apps output-dir]}]
  (let [get-app-dir-name #(last (s/split (:package-name %) #"\."))
        ...
        sorted-dirs (sort-by #(.getName %) test-dirs)]
    (doseq [test-dir sorted-dirs]
